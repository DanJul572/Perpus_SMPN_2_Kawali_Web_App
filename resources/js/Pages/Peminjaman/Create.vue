            ...
            <div class="col-md">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <ValidationObserver v-slot="{ handleSubmit }" ref="search_form">
                                    <form method="post" v-on:submit.prevent="handleSubmit(buku)">
                                        <ValidationProvider rules="required" v-slot="{ errors }">
                                            <div class="input-group">
                                                <input v-model="klasifikasi" type="text" placeholder="Klasifikasi Buku" :class="classInput(errors[0])">
                        ...
                        <div class="row">
                            <div class="col-md">
                                <div v-if="form.buku.length === 0" class="my-5">
                                ...
                                <table v-else class="table table-head-bg-primary mt-3">
                                    ...
                                    <tbody>
                                        <tr v-for="row in form.buku" :key="row.id">
...
export default {
    ...
    methods: {
        save () {
            if (this.form.buku.length == 0) {
                this.$awn.warning('Buku tidak boleh kosong');
            } else {
                this.$awn.asyncBlock(
                    axios.post(route('peminjaman.store'), this.form),
                    response => {
        ...
        buku () {
            this.$awn.asyncBlock(
                axios.get(route('buku.select', { klasifikasi: this.klasifikasi })),
                response => {
                    if (response.data.status == 'tidak ada') {
                        this.$awn.warning('Buku dengan klasifikasi "' + this.klasifikasi + '" tidak ditemukan.');
                    } else if (response.data.status == 'dipinjam') {
                        this.$awn.info('Buku dengan klasifikasi "' + this.klasifikasi + '" telah terpinjam.');
                    } else {
                        this.form.buku.push(response.data);
                    }
                    this.klasifikasi = '';
                    this.$refs.search_form.reset();
                },
        ...
        remove (id) {
            let index = this.form.buku.findIndex(x => x.id == id);
            this.form.buku.splice(index, 1);
        },
        cleanForm () {
            this.form.buku = [];
            this.form.tenggat = '';

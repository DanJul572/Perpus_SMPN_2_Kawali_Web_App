                        ...
                        <form method="post" v-on:submit.prevent="handleSubmit(save)">
                            <div class="modal-header">
                                <h6 class="modal-title" id="myModalTitle">
                                    <i class="fas fa-fw fa-tags"></i> Form Kategori
                                </h6>
...
export default {
    ...
    methods: {
        lists (page = 1) {
            this.$awn.asyncBlock(
                axios.get(route('kategori.lists') + '?page=' + page + '&keyword=' + this.keyword),
                response => {
                    if (response.data.data.length == 0) {
                        ...
                        this.$awn.success(response.data.data.length + ' kategori berhasil dimuat');
                    }
                },
                error => {
        ...
        save () {
            if (this.update_status == false) {
                this.$awn.asyncBlock(
                    axios.post(route('kategori.store'), this.form),
                    response => {
                       if (response.data.status == 'tersedia') {
                            this.$awn.info('Kategori dengan nama ' + this.form.nama + ' sudah tersedia');
                        } else {
                            this.lists();
                            this.cleanForm();
                            this.hideModal();
                            this.$awn.success('Kategori berhasil ditambahkan');
                        }
                    },
                    error => {
                        this.cleanForm();
                ...
                this.$awn.asyncBlock(
                    axios.post(route('kategori.update', {kategori: this.kategori_id}), this.form),
                    response => {
                        if (response.data.status == 'tersedia') {
        ...
        edit (id) {
            let kategori = this.rows.data.find(x => x.id == id);
        ...
        destroy (id) {
            let onOk = () => {
                this.$awn.asyncBlock(
                    axios.get(route('kategori.destroy', { kategori: id })),
                    response => {
                        if (response.data.relasi) {
                            this.$awn.info(response.data.relasi + ' buku dan mapel telah terkait kategori ini');
                        } else {
                            this.lists();
                            this.$awn.success('Kategori berhasil dihapus');
                        }
                    },
                    error => {
                        this.$awn.alert(error.message);
        ...
        async exportData () {
            const response = await axios.get(route('kategori.export'));

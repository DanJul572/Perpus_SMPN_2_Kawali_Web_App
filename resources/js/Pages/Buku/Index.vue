        ...
        <div class="row d-flex align-items-center mb-3">
            ...
            <div class="col text-right">
                ...
                <button v-on:click="[showModal('save'), cleanForm(), update_status = false]" class="btn btn-round btn-primary btn-sm">
                    <i class="fas fa-fw fa-plus"></i> Tambah Buku
                </button>
            </div>
        </div>
        <div class="row">
            <div class="col">
                ...
                <div v-else class="card">
                    <div class="card-header d-flex">
                        <h4 class="card-title mr-1">{{ buku_count }}</h4>
                        ...
                        <form method="post" v-on:submit.prevent="handleSubmit(save)">
                            <div class="modal-header">
                                <h6 class="modal-title" id="saveModalTitle">
                                    <i class="fas fa-fw fa-book"></i> Form Buku
                                </h6>
                    ...
                    <form method="post" v-on:submit.prevent="importExcel">
                        <div class="modal-header">
                            <h6 class="modal-title" id="importModalTitle">
                                <i class="fas fa-fw fa-book"></i> Import Buku
                            </h6>
...
export default {
    ...
    methods: {
        lists (page = 1) {
            this.$awn.asyncBlock(
                axios.get(route('buku.lists') + '?page=' + page + '&keyword=' + this.keyword),
                response => {
                    if (response.data.data.length == 0) {
                        ...
                        this.$awn.success(response.data.data.length + ' buku berhasil dimuat');
                    }
                },
                error => {
        ...
        formData () {
            this.$awn.asyncBlock(
                axios.get(route('buku.form')),
                response => {
                    this.kategori = response.data.kategori;
                    this.rak = response.data.rak;
                    this.buku_count = response.data.buku_count;
        ...
        save () {
            if (this.update_status == false) {
                this.$awn.asyncBlock(
                    axios.post(route('buku.store'), this.form),
                    response => {
                        if (response.data.status == 'tersedia') {
                            this.$awn.info('Buku dengan klasifikasi ' + this.form.klasifikasi + ' sudah tersedia');
                        } else {
                            this.lists();
                            this.cleanForm();
                            this.hideModal('save');
                            this.buku_count += 1;
                            this.$awn.success('Buku berhasil ditambahkan');
                        }
                    },
                    error => {
                        this.cleanForm();
                ...
                this.$awn.asyncBlock(
                    axios.post(route('buku.update', { buku: this.buku_id }), this.form),
                    response => {
                        if (response.data.status == 'tersedia') {
                            this.$awn.info('Buku dengan klasifikasi ' + this.form.klasifikasi + ' sudah tersedia');
                        } else if (response.data.status == 'tidak berubah') {
                            this.$awn.info('Data buku tidak berubah');
                        } else {
                            this.lists();
                            this.cleanForm();
                            this.hideModal('save');
                            this.$awn.success('Buku berhasil diubah');
                        }
                    },
                    error => {
                        this.cleanForm();
        ...
        edit (id) {
            let buku = this.rows.data.find(x => x.id == id);
            this.form.nama = buku.nama;
            this.form.klasifikasi = buku.klasifikasi;
            this.form.kategori_id = buku.kategori_id;
            this.form.rak = buku.rak;
        ...
        destroy (id) {
            let onOk = () => {
                this.$awn.asyncBlock(
                    axios.get(route('buku.destroy', { buku: id })),
                    response => {
                        if (response.data.relasi) {
                            this.$awn.info(response.data.relasi + ' peminjaman telah terkait buku ini');
                        } else {
                            this.lists();
                            this.buku_count -= 1;
                            this.$awn.success('Buku berhasil dihapus');
                        }
                    },
                    error => {
                        this.$awn.alert(error.message);
        ...
        importExcel () {
            if (this.import.length == 0) {
                ...
                this.$awn.asyncBlock(
                    axios.post(route('buku.import'), { rows: this.import }),
                    response => {
                        this.hideModal('import');
                        this.import = [];
                        this.formData();
                        this.lists();
                        this.buku_count += response.data.count_berhasil;
        ...
        async exportData () {
            const response = await axios.get(route('buku.export'));

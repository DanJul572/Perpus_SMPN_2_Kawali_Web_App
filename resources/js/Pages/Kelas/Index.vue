                        ...
                        <form method="post" v-on:submit.prevent="handleSubmit(save)">
                            <div class="modal-header">
                                <h6 class="modal-title" id="myModalTitle">
                                    <i class="fas fa-fw fa-door-open"></i> Form Kelas
                                </h6>
...
export default {
    ...
    methods: {
        lists (page = 1) {
            this.$awn.asyncBlock(
                axios.get(route('kelas.lists') + '?page=' + page + '&keyword=' + this.keyword),
                response => {
                    if (response.data.data.length == 0) {
                        this.$awn.warning('Tidak ada datang yang dimuat');
                    } else {
                        this.rows = response.data;
                        this.$awn.success(response.data.data.length + ' kelas berhasil dimuat');
                    }
                },
                error => {
        ...
        save () {
            if (this.update_status == false) {
                this.$awn.asyncBlock(
                    axios.post(route('kelas.store'), this.form),
                    response => {
                        if (response.data.status == 'tersedia') {
                            this.$awn.info('Kelas dengan kode ' + this.form.kode + ' sudah tersedia');
                        } else {
                            this.lists();
                            this.cleanForm();
                            this.hideModal();
                            this.$awn.success('Kelas berhasil ditambahkan');
                        }
                    },
                    error => {
                        this.cleanForm();
                ...
                this.$awn.asyncBlock(
                    axios.post(route('kelas.update', { kelas: this.kelas_id }), this.form),
                    response => {
                        if (response.data.status == 'tersedia') {
                            this.$awn.info('Kelas dengan nama ' + this.form.nama + ' sudah tersedia');
                        } else if (response.data.status == 'tidak berubah') {
                            this.$awn.info('Data kelas tidak berubah');
                        } else {
                            this.lists();
                            this.cleanForm();
                            this.hideModal();
                            this.$awn.success('Kelas berhasil diubah');
                        }
                    },
                    error => {
                        this.cleanForm();
        ...
        edit (id) {
            let kelas = this.rows.data.find(x => x.id == id);
            this.form.nama = kelas.nama;
        ...
        destroy (id) {
            let onOk = () => {
                this.$awn.asyncBlock(
                    axios.get(route('kelas.destroy', { kelas: id })),
                    response => {
                        if (response.data.relasi) {
                            this.$awn.info(response.data.relasi + ' kelas telah terkait kelas ini');
                        } else {
                            this.lists();
                            this.$awn.success('Kelas berhasil dihapus');
                        }
                    },
                    error => {
                        this.$awn.alert(error.message);
            ...
            this.$awn.confirm(
                'Kelas akan dihapus secara permanen',
                onOk,
                onCancle,
                {
                    labels: {

        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div v-if="mapel.length === 0" class="flex-center position-ref m-5">
                        ...
                        <table v-else class="table table-head-bg-primary">
                            ...
                            <tbody>
                                <tr v-for="row in mapel" :key="row.pivot.id">
                                    <td>{{ row.nama }}</td>
...
export default {
    ...
    methods: {
        detail () {
            this.$awn.asyncBlock(
                axios.get(route('peminjaman_mapel.detail', { siswa: this.id })),
                response => {
                    this.mapel = [];
                    this.total_denda = 0;
        ...
        save () {
            this.$awn.asyncBlock(
                axios.post(route('peminjaman_mapel.store', { siswa: this.id }), this.form),
                response => {
                    if (response.data.status == 'mapel tidak ada') {
                        this.$awn.info('Mapel dengan klasifikasi ' + this.form.klasifikasi + ' tidak tersedia');
                    } else if (response.data.status == 'terpinjam') {
                        this.$awn.info('Mapel dengan klasifikasi ' + this.form.klasifikasi + ' sudah terpinjam');
                    } else {
                        this.detail();
                        this.cleanForm();
                        this.hideModal('form');
        ...
        destroy (peminjaman_id) {
            let onOk = () => {
                this.$awn.asyncBlock(
                    axios.post(route('peminjaman_mapel.destroy', { siswa: this.id }), { peminjaman_id: peminjaman_id }),
                    () => {
                        this.detail();
        ...
        returned (peminjaman_id) {
            let onOk = () => {
                this.$awn.asyncBlock(
                    axios.post(route('peminjaman_mapel.returned', { siswa: this.id }), { peminjaman_id: peminjaman_id }),
                    () => {
                        this.detail();
        ...
        showInfo (id) {
            let mapel = this.mapel.find(x => x.pivot.id == id);
            this.info.klasifikasi = mapel.klasifikasi;
            this.info.nama = mapel.nama;
            this.info.kategori = mapel.kategori.nama;
            this.info.rak = mapel.rak;
            this.info.dikembalikan = mapel.pivot.dikembalikan;
            this.info.tenggat = moment(mapel.pivot.tenggat).format('D MMMM YYYY');
            let status = this.status(mapel.pivot.tenggat);
            this.info.status.negative = status.negative;
            this.info.status.positive = status.positive;
            this.info.denda = mapel.denda;
            this.info.tersimpan = moment(mapel.pivot.created_at).format('D MMMM YYYY');
        },
        mappingData (data) {
